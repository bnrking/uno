...
func TestShuffleCards(t *testing.T) {
	deck := shuffleCards([]model.Card{model.Card{Color: "red", Value: "1"}, model.Card{Color: "blue", Value: "2"}, model.Card{Color: "green", Value: "3"}}) //Shuffling test deck
	assert.NotEqual(t, deck[:0], model.Card{Color: "red", Value: "1"})
}

func TestPrintCard(t *testing.T) {
	card := model.Card{Color: "red", Value: "1"}
...
func TestPrintCards(t *testing.T) {
	deck := []model.Card{model.Card{Color: "red", Value: "1"}, model.Card{Color: "blue", Value: "2"}, model.Card{Color: "green", Value: "3"}}
	re := captureOutput(func() {
		printCards(deck)
	})
	assert.Equal(t, "red 1\nblue 2\ngreen 3\n", re)
